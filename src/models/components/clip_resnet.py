...
from src.MultiLabel.Non_Lightning.SSTandSAR.model.backbone.resnet import resnet101
...
class SCPNet(nn.Module):
    ...
    def __init__(self, classnames, clip_model, device):
        ...
        self.relation = torch.Tensor(np.load("/home/dan/LabelNoise/src/MultiLabel/Non_Lightning/SCPNet/relation+voc.npy"))
    ...
    def forward(self, image, device):
        ...
        # this will return both image features
        image_features, class_features = self.image_encoder(image.type(self.dtype))
        image_features = image_features / image_features.norm(dim=-1,
                                                              keepdim=True)
        logit_scale = self.logit_scale.exp()
        prompts = self.prompt_learner()
        text_features = self.text_encoder(prompts, tokenized_prompts, device)
        ...
        #logits = logit_scale * image_features @ label_embed.t()
